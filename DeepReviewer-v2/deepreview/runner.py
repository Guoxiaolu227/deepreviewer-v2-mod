...
async def _run_single_agent_instance(
    *,
    job_id: str,
    run_index: int,
    agent: Agent,
    ...
    """Run one independent agent loop instance and return its result.

    This is a self-contained copy of the agent-loop logic that does NOT
    share mutable state with the original run_job_async path.
    """
    from deepreview.meta_review import ReviewRunResult

    max_attempts = max(1, min(2, int(settings.agent_resume_attempts)))
    next_input: str | list = prompt
    usage_totals = {"requests": 0, "input_tokens": 0, "output_tokens": 0, "total_tokens": 0}

    def _consume(run_result, *, output_tag: str) -> str:
        ...
        if final_output_text:
            tag = f"run_{run_index + 1}_{output_tag}"
            write_text_atomic(
                Path(runtime.job_dir / f"agent_final_output_{tag}.txt"),
                final_output_text,
            )
    ...
    for attempt in range(1, max_attempts + 1):
                ...
                *run_result.to_input_list(),
                {
                    "role": "user",
                    "content": (
                        "MANDATORY ACTION NOW: Call review_final_markdown_write "
                        "in section mode. Submit one required section per call. "
                        "Do not output plain text. If retry_required, follow "
                        "next_steps and retry review_final_markdown_write."
                    ),
                },
            ]
            for forced_choice in ["review_final_markdown_write", "required"]:
                if runtime.final_markdown_text:
                    break
                try:
                    forced_agent = Agent(
                    ...
                    if runtime.final_markdown_text:
                        break
                    continue
                _consume(forced_result, output_tag=f"attempt_{attempt}_forced")
                if runtime.final_markdown_text:
                    break
                forced_input = [
                    *forced_result.to_input_list(),
                    {"role": "user", "content": "Continue section-mode submission."},
                ]
            break
        ...
        continuation = (
            ...
            f"annotations={runtime.annotation_count}, "
            f"paper_search_total_calls={usage.total_calls}, "
            f"distinct_queries={usage.distinct_queries}.\n"
            "If gates met, go to final report assembly in section mode."
        )
        next_input = [
            *run_result.to_input_list(),
    ...
    report_md = runtime.final_markdown_text or ""
    return ReviewRunResult(
        run_index=run_index,
        report_markdown=report_md,
        annotation_count=runtime.annotation_count,
        paper_search_total_calls=runtime.paper_search_usage.total_calls,
        paper_search_distinct_queries=runtime.paper_search_usage.distinct_queries,
        token_usage=dict(usage_totals),
        tool_usage=dict(runtime.tool_counts),
    )
...
async def run_job_async(job_id: str) -> None:
    ...
    if num_parallel > 1 and settings.enable_meta_review:
        await _run_parallel_pipeline(
            job_id=job_id,
            job=job,
...
async def _run_parallel_pipeline(
    *,
    ...
    num_reviews = max(2, min(8, int(settings.num_parallel_reviews)))
    append_event(job_id, "parallel_reviews_starting", num_reviews=num_reviews)
    job_dir = Path(artifacts["source_pdf"]).parent

    # Build shared prompt and config
    prompt = build_review_agent_system_prompt(
        source_file_id=job_id,
        source_file_name=job.source_pdf_name,
        ui_language=settings.ui_language,
    ...
    async def _run_one(index: int) -> ReviewRunResult:
        runtime = ReviewRuntimeContext(
            job_id=job_id,
            job_dir=job_dir,
            page_index=page_index,
        ...
        tools = build_review_tools(runtime)
        agent = Agent(
            name=f"DeepReviewer2Agent-R{index + 1}",
            instructions=prompt,
            tools=tools,
            model=agent_model,
            model_settings=_build_agent_model_settings(),
        ...
        try:
            result = await _run_single_agent_instance(
                job_id=job_id,
                run_index=index,
                agent=agent,
                run_config=run_config,
            ...
            append_event(
                job_id, "parallel_run_failed",
                run_index=index + 1,
                error=f"{type(exc).__name__}: {exc}",
            )
            result = ReviewRunResult(
                run_index=index,
                report_markdown=f"[Run #{index + 1} failed: {type(exc).__name__}: {exc}]",
                annotation_count=0,
                paper_search_total_calls=0,
                paper_search_distinct_queries=0,
            )
        return result

    tasks = [_run_one(i) for i in range(num_reviews)]
    run_results = await asyncio.gather(*tasks)
    run_results = [r for r in run_results if isinstance(r, ReviewRunResult)]

    append_event(
        job_id, "parallel_reviews_all_completed",
    ...
    for rr in run_results:
        if rr.report_markdown and not rr.report_markdown.startswith("[Run #"):
            save_individual_report(
                ...
                report_markdown=rr.report_markdown,
            )

    # Meta-review merge
    set_status(
        job_id, JobStatus.final_report_persisting,
        f"Running meta-review merge over {len(run_results)} reports...",
    )
    meta = await merge_reports(
    ...
    save_consolidated_report(
        job_dir=job_dir,
        report_markdown=meta.consolidated_markdown,
        is_final=True,
    )

    # Accumulate usage
    parallel_requests = sum(r.token_usage.get("requests", 0) for r in run_results)
    parallel_input = sum(r.token_usage.get("input_tokens", 0) for r in run_results)
    parallel_output = sum(r.token_usage.get("output_tokens", 0) for r in run_results)
    parallel_total = sum(r.token_usage.get("total_tokens", 0) for r in run_results)
    parallel_requests += meta.merge_token_usage.get("requests", 0)
    parallel_input += meta.merge_token_usage.get("input_tokens", 0)
    parallel_output += meta.merge_token_usage.get("output_tokens", 0)
    parallel_total += meta.merge_token_usage.get("total_tokens", 0)

    def apply_parallel_usage(state):
        state.annotation_count = sum(r.annotation_count for r in run_results)
        state.usage.token.requests = parallel_requests
        state.usage.token.input_tokens = parallel_input
        state.usage.token.output_tokens = parallel_output
        state.usage.token.total_tokens = parallel_total
        state.usage.paper_search.total_calls = sum(
            r.paper_search_total_calls for r in run_results
        )
        state.usage.paper_search.distinct_queries = max(
            (r.paper_search_distinct_queries for r in run_results), default=0
        )
        meta_info = dict(state.metadata)
        meta_info["meta_review_confirmed_count"] = meta.confirmed_count
        meta_info["meta_review_single_source_count"] = meta.single_source_count
        meta_info["meta_review_disputed_count"] = meta.disputed_count
        meta_info["meta_review_merge_token_usage"] = meta.merge_token_usage
        meta_info["num_parallel_reviews"] = num_reviews
        state.metadata = meta_info

    mutate_job_state(job_id, apply_parallel_usage)
    append_event(
        job_id, "meta_review_merge_completed",
        confirmed=meta.confirmed_count,
        single_source=meta.single_source_count,
        disputed=meta.disputed_count,
    ...
    if not skip_pdf:
        ...
        token_usage_for_pdf = {
            ...
            "input_tokens": parallel_input,
            "output_tokens": parallel_output,
            "total_tokens": parallel_total,
        }
        _render_report_pdf(
            job_id=job_id,
            job_title=job.title,
            ...
            source_pdf_path=source_pdf,
            report_pdf_path=report_pdf_path,
            annotations=[],  # individual annotations not aggregated for now
            content_list=parse_result.content_list,
            token_usage=token_usage_for_pdf,
            agent_model=str(settings.agent_model or "").strip(),
        )
    ...
    def apply_completed(state):
        state.status = JobStatus.completed
        state.message = (
            f"Review pipeline completed ({num_reviews} parallel reviews, meta-review merged)."
        )
        state.error = None
        state.final_report_ready = True
    ...
    append_event(
        job_id, "completed",
        report_pdf_path=str(report_pdf_path) if not skip_pdf else None,
        num_parallel_reviews=num_reviews,
